%% DEMO: synchronization to stable equilibrium
% author: Loi Do
% doloi@fel.cvut.cz

% Demo, showing synchronization to stable equilibrium of 5 pendulums
...
X0_idf =  repmat([0;0],N,1) + pi*repmat([1;0],N,1).*(rand(2*N,Ntraj) - 0.5);    % Initial states for all trajectories with random perturbations
Xref = repmat(kron(ones(N,1), [0;0]), 1, Tsim_idf/Ts + 1); % Reference for all pendulums and for whole time interval
...
f_name = ['./data/DEMO_data_stableEq_numTraj-' , num2str(Ntraj), '.mat'];
disp('Data collected');
...
Xsim = zeros(2*N, Nsim+1);
Xsim(:,1) = repmat([0;0],N,1) + pi*repmat([1;0],N,1).*(rand(2*N,1) - 0.5);  % Random initial angles of pendulums
